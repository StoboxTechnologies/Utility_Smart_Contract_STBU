...
contract STBU is ERC20, Ownable {
    ...
    function addAllowedContract(address _contract)
        external
        onlyOwner
        returns (bool)
    {
        require(
            ...
            "Address: is not contract or not deployed"
        );
        allowedContracts[_contract] = true;
        return true;
    }

    function removeAllowedContract(address _contract)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function transferFrom(
        ...
        address recipient,
        uint256 amount
    ) public override onlyAllowedContracts returns (bool) {
        return super.transferFrom(sender, recipient, amount);
    }

    function mint(uint256 amount) public onlyManager returns (bool) {
        require(
            managers[msg.sender].mint.max >=
                managers[msg.sender].mint.actual.add(amount),
            "Limit exceeded"
        );
        uint256 supply = totalSupply();
        require(_maxSupply >= supply.add(amount), "Supply exceeded cap");
        managers[msg.sender].mint.actual = managers[msg.sender].mint.actual.add(
            amount
        );
    ...
    function burn(uint256 amount) public onlyManager returns (bool) {
        require(
            managers[msg.sender].burn.max >=
                managers[msg.sender].burn.actual.add(amount),
            "Limit exceeded"
        );
        uint256 supply = totalSupply();
        require(_minSupply >= supply.sub(amount), "Supply exceeded cap");
        managers[msg.sender].burn.actual = managers[msg.sender].burn.actual.add(
            amount
        );
    ...
    function receiveStuckTokens(address token) public onlyOwner returns (bool) {
        ...
        require(
            coin.transfer(treasury, coin.balanceOf(address(this))),
            "Transfer: error"
        );
        return true;
    }
